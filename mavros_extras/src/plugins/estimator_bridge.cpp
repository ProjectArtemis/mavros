 ...
 *
 * Copyright 2016 Mohammed Kabir, UASys LLC.
 *
 * This will publish :
...
public:
	...
	void initialize(UAS &uas_)
	{
		...
		fcu_global_orient_pub = estimator_bridge_nh.advertise<sensor_msgs::Imu>("global_orientation", 10);
	...
	Subscriptions get_subscriptions() {
		return {
			       ...
			       make_handler(&EstimatorBridgePlugin::handle_gps_raw_cov)
		};
	}
...
private:
	...
	ros::Publisher fcu_global_orient_pub;
	...
	{
		auto att_quaternion = ftf::transform_orientation_aircraft_baselink(
					Eigen::Quaterniond(att_q.q1, att_q.q2, att_q.q3, att_q.q4));

		auto att_angular_vel = ftf::transform_frame_aircraft_baselink(
				Eigen::Vector3d(att_q.rollspeed, att_q.pitchspeed, att_q.yawspeed));

		auto imu_msg = boost::make_shared<sensor_msgs::Imu>();
		
		// XXX Hack to rotate to ENU frame
		tf::Quaternion rotation = tf::createQuaternionFromYaw (90.0 * DEG_TO_RAD);
		tf::Quaternion imu_orientation;
		tf::quaternionEigenToTF(att_quaternion, imu_orientation);
		imu_orientation *= rotation;

		// fill
		imu_msg->header = m_uas->synchronized_header("base_link", att_q.time_boot_ms);

		tf::quaternionTFToMsg(imu_orientation, imu_msg->orientation);
		tf::vectorEigenToMsg(att_angular_vel, imu_msg->angular_velocity);

		// publish
		fcu_global_orient_pub.publish(imu_msg);
	}
	...
	void handle_highres_imu(const mavlink::mavlink_message_t *msg, mavlink::common::msg::HIGHRES_IMU &imu_hr)
	{
		...
		if (imu_hr.fields_updated & ((7 << 3) | (7 << 0))) {
			...
			auto raw_gyro = ftf::transform_frame_aircraft_baselink(Eigen::Vector3d(imu_hr.xgyro, imu_hr.ygyro, imu_hr.zgyro));
			auto raw_accel = ftf::transform_frame_aircraft_baselink(Eigen::Vector3d(imu_hr.xacc, imu_hr.yacc, imu_hr.zacc));
			...
			tf::vectorEigenToMsg(filtered_gyro, imu_msg->angular_velocity);
			tf::vectorEigenToMsg(filtered_accel, imu_msg->linear_acceleration);
	...
	void handle_gps_raw_cov(const mavlink::mavlink_message_t *msg, mavlink::common::msg::GPS_RAW_COV &gps)
	{
	
		auto fix = boost::make_shared<sensor_msgs::NavSatFix>();

		fix->latitude = gps.lat * 1e-7;		// deg
		fix->longitude = gps.lon * 1e-7;	// deg
		fix->altitude = gps.alt * 1e-3;		// m
		
		double ecef_datum[3];
		
		bool gps_good = ((gps.fix_type > 2)
			      && (gps.satellites_visible > MIN_NUM_SATS)
			      && (sqrtf(gps.position_cov[2]) < EPV_THRESHOLD)
		    	      && (sqrtf(fmax(gps.position_cov[0], gps.position_cov[1])) < EPH_THRESHOLD));
		...
		if (!gps_inited && gps_good && vio_inited)
		{
			...
			origin.latitude = LLH_datum.latitude * 1e7;
			origin.longitude = LLH_datum.longitude * 1e7;
			origin.altitude = LLH_datum.altitude * 1e3;
		...
		auto vel = ftf::transform_frame_ned_enu(Eigen::Vector3d(gps.ned_velocity[0],
						gps.ned_velocity[1],
						gps.ned_velocity[2]));
		...
		gps_odom_msg->pose.covariance[0] = gps.position_cov[1];
		gps_odom_msg->pose.covariance[7] = gps.position_cov[0];
		gps_odom_msg->pose.covariance[14] = gps.position_cov[2];
		gps_odom_msg->twist.covariance[0] = gps.velocity_cov[1];
		gps_odom_msg->twist.covariance[7] = gps.velocity_cov[0];
		gps_odom_msg->twist.covariance[14] = gps.velocity_cov[2];
	 ...
	 *	3. Fix hacky rotatations of attitude/position
	 *	4. Properly fill covariance of pos/vel/acc
	 *	5. Fix hacky controller setpoint resets properly
	 *
	 */
	...
	void vio_odometry_cb(const nav_msgs::Odometry::ConstPtr &odom) {
		...
		auto quaternion = ftf::transform_orientation_baselink_aircraft(Eigen::Quaterniond(pose.rotation()));
		auto position = ftf::transform_frame_baselink_aircraft(Eigen::Vector3d(pose.translation()));
		...
		auto linear_velocity = ftf::transform_frame_enu_ned(vel);
		auto angular_velocity = ftf::transform_frame_enu_ned(ang);
		
		// Fill in ATTITUDE_QUATERNION_COV
		
		// XXX Hack to rotate to NED
		tf::Quaternion rotation = tf::createQuaternionFromYaw (90.0 * DEG_TO_RAD);
		tf::Quaternion vio_orientation;
		tf::quaternionEigenToTF(quaternion, vio_orientation);
		vio_orientation *= rotation;
		tf::quaternionTFToEigen(vio_orientation, quaternion);
		
		Eigen::Matrix3d R = quaternion.toRotationMatrix();
		
		mavros::ftf::quaternion_to_mavlink(quaternion, att.q);
		
		//Eigen::Vector3d global_angular_vel = R * angular_velocity;
		...
		// Covert to global frame velocity
		Eigen::Vector3d global_velocity = R * linear_velocity;
		
		local.vx = global_velocity.x();
		local.vy = global_velocity.y();
		local.vz = global_velocity.z();
		
		// XXX Hack to rotate to NED
		tf::quaternionTFToEigen(rotation, quaternion);
		R = quaternion.toRotationMatrix();
		Eigen::Vector3d position_rotated = R * position;
		
		local.estimator_type = (uint8_t)mavlink::common::MAV_ESTIMATOR_TYPE::VIO;
		
		local.x = position_rotated.x();
		local.y = position_rotated.y();
		local.z = position_rotated.z();
		
		// XXX Hack till we get the frame coversions working
		local.covariance[0] = odom->pose.covariance[7];
		local.covariance[1] = odom->pose.covariance[0];
		local.covariance[2] = odom->pose.covariance[14];
		local.covariance[3] = odom->twist.covariance[7];
		local.covariance[4] = odom->twist.covariance[0];
		local.covariance[5] = odom->twist.covariance[14];
